...
Всего сервис имеет 4 микросервиса:
...
**Микросервис пользователя User** предназначен для получения и изменения данных пользователей. Вследствие чего он имеет доступ к PostgreSQL, которая хранит данные о пользователях. Помимо этого для решения поисковых и аналитических задач, микросервис пользователя взаимодействует с ElasticSearch и ClickHouse.

**Микросервис статьи Article** отвечает за хранение и обработку статей. Запрос на чтение API-Gateway направляет напрямую в микросервис, в то время как запис производится через Kafka, что позволяет сделать очередь на публикацию и обновление статей и обновлять информацию асинхронно. Для хранения данных микросервис статьи взаимодействует с PostgreSQL, а для решения поисковых и аналитических задач - с ElasticSearch и ClickHouse соответственно.

**Микросервис комментария Comment** предназначен для чтения и отправки комментариев к статьям. Запрос на запись проходит через Kafka, что позволяет сделать очередь на публикацию комментариев, а также это позволит получать уведомление об успешной публикации, либо понимать, из-за какой ошибки не произошла публикация. Для хранения данных о комментариях микросервис взаимодействует с PostgreSQL.
