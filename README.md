...
Поскольку Википедия имеет большое количество статического контента, то принято решение использовать 2 датацентра и CDN серверы. 2 датацентра будут работать в Москве в паре, резервируя друг друга, что позволит обеспечить должную надежность, а CDN будут расположены по всей территории России, позволяя уменьшить время отклика и повысить удобство пользования сервисом ([подробнее о балансировке нагрузки](#4_part)).

Так как большая часть аудитории Википедии располагается в России, то все CDN располагать будем также в России.

Из-за неравномерного распределения плотности населения России большая часть CDN будет располагаться в европейской части. Но из-за большой протяженности России требуется ЦОД, который будет рассчитан на трафик с Сибири и Дальнего Востока.
...
На их основе выберем города, которые лучше всего подойдут под расположение CDN. Будем учитывать раположение, плотность населения, наличие большого количества магистральных сетей связи.
...
Время отклика не превышает 100 мс, что не критично для сервиса онлайн энциклопедии. Это означает, что раположение CDN выбрано удачно.

#### Нагрузка на ЦОД-ы

Рассчитаем предполагаемую нагрузку на ЦОД-ы. Будем исходить из того, что RPD на MVP составляет 39 000 000. Рассмотрим плотность населения России и стран СНГ и распределение регионов по серверам, и на этой основе рассчитаем преполагаемое количество RPD на каждый из CDN.
...
| Датацентр             | Пользователи, человек | Пользователи, % | Нагрузка (RPD) |
| --------------------- | --------------------- | --------------- | -------------- |
| Москва (ЦОД)          | 30 300 000            | 23              | 8 970 000      |
| Санкт-Петербург (CDN) | 15 060 000            | 12              | 4 680 000      |
| Ростов-на-Дону (CDN)  | 29 870 000            | 22              | 8 580 000      |
| Екатеринбург (CDN)    | 29 250 000            | 22              | 8 580 000      |
| Иркутск (CDN)         | 28 640 000            | 21              | 8 190 000      |
...
| Компонент   | Способ(ы) резервирования                          | Обоснование                                                                                                                                                                                                                                                                                                                                                |
| ----------- | ------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Nginx       | Резервирование ресурсов и физических компонентов. | Критически важно, чтобы Nginx оставался в рабочем состоянии, поэтому предлагается его дублирование. Под дублированием подразумевается полная копия Nginx. Два Nginx работают, резервируя друг друга: таким образом, при отказе первого, второй будет подменять его на время устранения неисправностей.                                                     |
| API-Gateway | Резервирование логики.                            | Аналогично Nginx-у имеет смысл реплицировать главный шлюз для повышения отказаустойчивости и пропускной способности.                                                                                                                                                                                                                                       |
| Redis       | Резервирование                                    | Redis хранит сессии пользователей для авторизации, поэтому требуется обеспечить должную отказоустойчивость. С другой стороны потеря сессий не так критична, поэтому было принято решение отказаться от репликации.                                                                                                                                         |
| PostgreSQL  | Репликация                                        | В случае с БД крайне важно иметь постоянный и надежный доступ к данным, поэтому они будут храниться в нескольких копиях, что позволит избежать потерю данных в случае отказа одной из БД.                                                                                                                                                                  |
| CEPH        | Репликация                                        | Ceph автоматически распределяет данные (в нашем случае фотографии) по узлам в кластере, обеспечивая, что данные не хранятся в одной точке. Любые данные копируются в трех узлах кластера Ceph. Это означает, что даже если один из узлов отказывает, фотографии остаются доступными для чтения и записи, и система продолжает работать в привычном режиме. |

### Другие методы постоения отказоустойчивой архитектуры

#### Failover policy

Для контроля работоспособности узлов в кластере будем передавать сигнал "пульса" от одного узла к другому. По наличию этого сигнала управляющее будем судить о нормальной работе узлов. Это позволяет быстро обнаружить сбои и инициировать процесс переключения на резервные узлы.

Failover policy позволит:

- Уменьшить запросы на проблемный узел: в случае временной недоступности какого-либо сервиса, система автоматически уменьшает количество запросов на этот компонент, чтобы избежать возможной нагрузки и предотвратить негативное влияние на производительность системы.

- Автоматизировать процесс: Failover обеспечивает автоматическое переключение на резервные сервера без необходимости вмешательства человека, что делает процесс более эффективным и надежным.

#### Graceful shutdown

Позволит системе закрыть все активные соединения, завершить обработку текущих задач и корректно освобождить ресурсы, прежде чем завершить работу, в следствие чего уменьшится количество ошибок.
...
Поскольку не планируется разработка каких-либо клиентов помимо web, то все запросы будут идти именно из браузерного клиента.

Основным узлом является API-Gateway, который маршрутизирует запросы между микросервисами.

Всего сервис имеет 5 микросервисов:

- микросервис авторизации Authorization
- микросервис пользователя User
- микросервис статьи Article
- микросервис комментария Comment
- микросервис поиска Search

**Микросервис авторизации Authorization** отвечает за авторизацию пользователей. Поэтому он взаимодействует с Redis, которая хранит сессии пользователей.

**Микросервис пользователя User** предназначен для получения и изменения данных пользователей. Вследствие чего он имеет доступ к PostgreSQL, которая хранит данные о пользователях. Помимо этого для решения аналитических задач, микросервис пользователя взаимодействует с ClickHouse.

**Микросервис статьи Article** отвечает за хранение и обработку статей. Запрос на чтение API-Gateway направляет напрямую в микросервис, в то время как запись производится через Kafka, что позволяет сделать очередь на публикацию и обновление статей и обновлять информацию асинхронно. Для хранения данных микросервис статьи взаимодействует с PostgreSQL, а для решения аналитических задач - с ClickHouse.

**Микросервис комментария Comment** предназначен для чтения и отправки комментариев к статьям. Запрос на запись проходит через Kafka, что позволяет сделать очередь на публикацию комментариев, а также это позволит получать уведомление об успешной публикации, либо понимать, из-за какой ошибки публикация не произошла. Для хранения данных о комментариях микросервис взаимодействует с PostgreSQL.

**Микросервис поиска Search** предназначен для решения поисковых задач, вследствие чего он взаимодействует с ElasticSearch. Любой запрос на поиск статьи или пользователя идет именно к данному микросервису. При создании и обновлении статьи или пользователя, обновление происходит сначала в PostgreSQL, после чего ElasticSearch индексирует данные. Таким образом при поиске мы будем получать актуальную информацию.

**За статикой в виде фотографий будем ходить напрямую в CEPH**

---

## 11. Расчет ресурсов <a name="11_part"></a>

### Рассмотрим нагрузку на каждый из микросервисов:

(будем рассчитывать пиковые показатели как средние\*1.67)

| Микросервис   | Пиковый RPS | Пиковый трафик | Характер сервиса      | CPU | RAM   |
| ------------- | ----------- | -------------- | --------------------- | --- | ----- |
| Authorization | 1           | 400 Кб/с       | Средняя бизнес-логика | 2   | 2 ГБ  |
| User          | 2           | 640 Кб/с       | Средняя бизнес-логика | 2   | 2 ГБ  |
| Article       | 580         | 47 Мб/с        | Тяжелая бизнес-логика | 58  | 58 ГБ |
| Comment       | 2           | 2,2 Мб/с       | Средняя бизнес-логика | 2   | 2 ГБ  |
| Search        | 167         | 14 Мб/с        | Тяжелая бизнес-логика | 17  | 17 ГБ |

Поскольку мы используем также 4 CDN сервера, то рассчитаем нагрузку на 1 из них. Максимальный рассчетный RPD на CDN равен 8 580 000, поэтому пиковый RPS равен 166.

| CDN | Пиковый RPS | Пиковый трафик | Характер сервиса | CPU | RAM  |
| --- | ----------- | -------------- | ---------------- | --- | ---- |
| CDN | 166         | 13 Мб/с        | CDN              | 2   | 2 ГБ |

Будем брать минимум 2 сервера на случай отказа одного из них.

### Выберем конфигурацию железа для каждого из микросервисов:

Поскольку в проекте предполагается использование 2-х датацентров (один резервирует другой) и 4-х CDN, то нам потребуется мощное железо (в количестве двух экземпляров), способное выдерживать вышеуказанные нагрузки, и 4 менее мощных комплекта железа для CDN.

#### Железо для датацентров

| Микросервис   | Хостинг | Конфигурация                                                              | Cores | Count | Покупка, $ | Аренда, $ |
| ------------- | ------- | ------------------------------------------------------------------------- | ----- | ----- | ---------- | --------- |
| Authorization | own     | A2SDi-8C-HLN4F-8-Core C3758 Atom/2x 4GB 2400MHz DDR4/CyberServe Atom-104i | 8     | 2     | 4 500      | 50        |
| User          | own     | A2SDi-8C-HLN4F-8-Core C3758 Atom/2x 4GB 2400MHz DDR4/CyberServe Atom-104i | 8     | 8     | 4 500      | 50        |
| Article       | own     | A2SDi-8C-HLN4F-8-Core C3758 Atom/2x 4GB 2400MHz DDR4/CyberServe Atom-104i | 8     | 2     | 4 500      | 50        |
| Comment       | own     | A2SDi-8C-HLN4F-8-Core C3758 Atom/2x 4GB 2400MHz DDR4/CyberServe Atom-104i | 8     | 2     | 4 500      | 50        |
| Search        | own     | A2SDi-8C-HLN4F-8-Core C3758 Atom/2x 4GB 2400MHz DDR4/CyberServe Atom-104i | 8     | 3     | 4 500      | 50        |

#### Железо для CDN

| CDN | Хостинг | Конфигурация                                                              | Cores | Count | Покупка, $ | Аренда, $ |
| --- | ------- | ------------------------------------------------------------------------- | ----- | ----- | ---------- | --------- |
| CDN | own     | A2SDi-8C-HLN4F-8-Core C3758 Atom/2x 4GB 2400MHz DDR4/CyberServe Atom-104i | 8     | 2     | 4 500      | 50        |

### Рассчитаем затраты на железо:

#### Покупка:

1. Стоимость одного **датацентра:**

4 500\*2 + 4 500\*8 + 4 500\*2 + 4 500\*2 + 4 500\*3 = 76 500 $

Будем предполагать обновление железа 1 раз в 5 лет, тогда стоимость в год будет:

72 000 / 5 = 15 300 $

Поскольку у нас 2 датацентра, то общая стоимость железа в год будет **36 600 $**

2. Стоимость одного **CDN:**
...
Поскольку у нас 4 CDN, то общая стоимость железа в год будет:
1 800 \* 4 = **7 200 $**

#### Аренда:

1. Рассчитаем стоимость аренды одного **датацентра** за год:

12 \* (50\*2 + 50\*8 + 50\*2 + 50\*2 + 50\*3) = 10 200 $

Поскольку у нас 2 датацентра, то общая стоимость аренды в год будет **20 400 $**

2. Рассчитаем стоимость аренды одного **CDN** за год:

12 \* 50\*2 = 1 200 $

Поскольку у нас 4 CDN, то общая стоимость аренды в год будет:

4 \* 1 200 = **4 800 $**

#### Вывод

- Общая стоимость затрат при покупке: **43 800 $**

- Общая стоимость затрат при аренде: **25 200 $**

Из полученных рассчетов сделаем вывод, что **аренда железа будет выгоднее покупки.**
