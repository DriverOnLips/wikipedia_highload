...
Поскольку Википедия имеет большое количество статического контента, то принято решение использовать 2 датацентра и CDN серверы. 2 датацентра будут работать в Москве в паре, резервируя друг друга, что позволит обеспечить должную надежность, а CDN серверы будут расположены по всей территории россии, позволяя уменьшить время отклика и повысить удобство пользования сервисом ([подробнее о балансировке нагрузки](#4_part)).
...
| User          | 2           | 640 Кб/с       | Тяжелая бизнес-логика | 2   | 2 ГБ  |
| Article       | 749         | 60 Мб/с        | Тяжелая бизнес-логика | 76  | 76 ГБ |
| Comment       | 2           | 2,2 Мб/с       | Тяжелая бизнес-логика | 2   | 2 ГБ  |
...
| User          | own     | A2SDi-8C-HLN4F-8-Core C3758 Atom/2x 4GB 2400MHz DDR4/CyberServe Atom-104i | 8     | 10    | 4 500      | 50        |
...
4 500\*2 + 4 500\*10 + 4 500\*2 + 4 500\*2 = 72 000 $

Российская Википедия была создана в 2001 году. Предположим, что требуется обновление оборудования раз в 10 лет. Тогда стоимость покупки оборудования будет равна:

72 000 \* 3 = 216 000 $

#### Аренда:

Рассчитаем стоимость аренды за время существования сервиса:

12 \* 24 \* (50\*2 + 50\*10 + 50\*2 + 50\*2) = 230 400 $

Из полученных рассчетов сделаем вывод, что покупка серверов будет выгоднее аренды.
