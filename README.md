...
**Микросервис пользователя User** предназначен для получения и изменения данных пользователей. Вследствие чего он имеет доступ к PostgreSQL и CEPH, которые хранят данные о пользователях и их аватары соответственно. Помимо этого для решения поисковых и аналитических задач, микросервис пользователя взаимодействует с ElasticSearch и ClickHouse.

**Микросервис статьи Article** отвечает за хранение и обработку статей. Запрос на чтение API-Gateway направляет напрямую в микросервис, в то время как запис производится через Kafka, что позволяет сделать очередь на публикацию и обновление статей и обновлять информацию асинхронно. Для хранения данных микросервис статьи взаимодействует с PostgreSQL и с CEPH, а для решения поисковых и аналитических задач - с ElasticSearch и ClickHouse соответственно.
